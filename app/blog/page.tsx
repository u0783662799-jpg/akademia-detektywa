      ...
      <main id="main-content">
        ...
        <section className="wrap journal" aria-label="Artykuły dla rodziców">
          ...
          <div className="journal-list">
            {rest.map((article,i)=><article key={article.slug}><span className="journal-number">0{i+2}</span><div><p className="section-label">{article.category} · {article.readTime}</p><h2><Link href={`/blog/${article.slug}`}>{article.title}</Link></h2><p>{article.desc}</p><time dateTime={article.date}>{new Date(article.date).toLocaleDateString("pl-PL",{ day:"numeric",month:"long",year:"numeric" })}</time></div><Link href={`/blog/${article.slug}`} className="read-arrow" aria-label={`Czytaj: ${article.title}`}><ArrowRight size={24} /></Link></article>)}
          </div>
        </section>
