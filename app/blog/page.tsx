      ...
      <main id="main-content">
        ...
        <section className="wrap journal" aria-label="Artykuły dla rodziców">
          <article className="journal-feature">
            <Link href={`/blog/${featured.slug}`} className={`journal-image${featured.image ? " journal-image-photo" : ""}`} aria-label={featured.title}><Image src={featured.image?.src ?? "/iwo-puzzle-suspects.webp"} alt={featured.image?.alt ?? "Podejrzani i wskazówki z darmowej sprawy PDF Akademii"} width={featured.image?.width ?? 990} height={featured.image?.height ?? 1400} priority sizes="(max-width: 700px) 90vw, 430px" /></Link>
